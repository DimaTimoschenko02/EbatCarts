...
const PARAM_DESCRIPTORS: ParamDescriptor[] = [
  ...
  { key: "accelForce", group: "Скорость", min: 10, max: 100, step: 2, unit: "m/s²",
    short: "Сила мотора. Низкое (15-17) — разгон ленивый, чувствуется вес (дефолт 17, разгон до ~8.5 м/с ~30% дольше чем раньше). Среднее (25-35) — аркадный стандарт. Высокое (50+) — моментальный разгон, пропадает ощущение веса. Осторожно: терминальная скорость жёстко завязана на kDrag/kRolling — если опустить ниже ~17, машина может вообще перестать разгоняться выше ~9.5 м/с (см. комментарий в types.ts)." },
  ...
  { key: "reverseRatio", group: "Скорость", min: 0.2, max: 1.0, step: 0.05, unit: "x",
    short: "Сила заднего хода относительно переднего. 0.3 — еле ползёт, 0.5 — удобно для манёвров, 0.88 (дефолт) — почти как вперёд, задний ход с рулём реально едет а не тонет в собственном сопротивлении повороту (см. reverseCorneringDragFloor)." },
  ...
  { key: "steerSlewRateIn", group: "Сглаживание ввода", min: 1, max: 20, step: 0.5, unit: "/s",
    short: "Как быстро руль докручивается до упора при нажатии A/D. 1.4 (дефолт) — заметный вес руля (~0.7с до упора, машина не дёргается в поворот сразу как нажал), 2.2 — было раньше, чуть бодрее (~0.45с), 5 — отзывчиво, 10+ — почти как тумблер." },
  ...
  { key: "steerLowSpeedMult", group: "Рулёжка", min: 0.5, max: 3, step: 0.05, unit: "x",
    short: "Насколько острее руль на малой скорости (для парковки/манёвров). 0.72 (дефолт, было 0.9) — заметно спокойнее чем раньше, поворот (и обычный, и в дрифте) ощутимо мягче — вместе с maxSteerAngleDeg это была главная причина 'машина крутится втрое резче эталона'. 1.0 — без разницы, 2.0 — очень острый разворот на месте." },
  { key: "steerHighSpeedMult", group: "Рулёжка", min: 0.1, max: 1.5, step: 0.05, unit: "x",
    short: "Насколько руль 'успокаивается' на максимальной скорости. 0.62 (дефолт, было 0.85) — плавные широкие повороты на скорости. 0.5 — ещё шире, 1.0 — руль одинаково острый везде." },

  // ── Bicycle v3.0 ─────────────────────────────────────────────────────
  { key: "maxSteerAngleDeg", group: "Bicycle v3.0", min: 10, max: 55, step: 1, unit: "°",
    short: "Максимальный угол передних колёс — главная ручка остроты руля. 16° (дефолт, было 28°) — эталонный SmashKarts-радиус дрифта (~3.6м вместо 1.3м), обычный поворот тоже заметно мягче. 20° — рулится ощутимо острее. 28° — как было раньше (втрое круче эталона). 35°+ — очень острая, 45°+ — почти дёрганая." },
  ...
  { key: "corneringDragDriftMult", group: "Дрифт: сигналы и эффекты", min: 1.0, max: 8.0, step: 0.25, unit: "x",
    short: "Во сколько раз усиливается торможение из corneringDragCoeff, когда машина реально в активном дрифте (не в обычном повороте). 1.0 — дрифт тормозит так же как обычный поворот, 5.5 (дефолт, было 4) — в дрифте машина заметнее теряет скорость сразу на входе, вне дрифта тормозит слабо." },
  { key: "driftPenaltyTau", group: "Дрифт: сигналы и эффекты", min: 0.1, max: 3.0, step: 0.05, unit: "s",
    short: "Насколько 'инерционно' торможение из заноса реагирует на реальный занос задней оси (используется для трёх штрафов выше). Маленькое (0.1-0.2) — машина резко проседает по скорости прямо в момент входа в занос, потом отпускает (ощущение 'сброса' от заноса). 1.0 (дефолт, было 2.0) — просадка на входе быстрее и заметнее чем раньше, но ещё без резкого рывка. 2.0-3.0 — торможение почти не успевает включиться за короткий дрифт." },
  ...
  { key: "driftYawBonus", group: "Дрифт v4.0 (continuous)", min: 0.0, max: 4.0, step: 0.1, unit: "rad/s",
    short: "Доп. угловая скорость в дрифте — машина 'довинчивается' резче чем чистая физика. 0.2 (дефолт, было 1.5) — лёгкий довинт поверх физики шин; на старом значении это ОДНО было почти половиной всей скорости поворота в дрифте (машина крутилась втрое резче эталона SmashKarts). 0 — совсем без довинчивания, 3 — легко закрутить машину на 540°." },
  ...
  { key: "camYawFollowRate", group: "Камера", min: 1, max: 15, step: 0.5, unit: "/s",
    short: "Как быстро камера доворачивается за машинкой, когда та меняет направление движения (например, в дрифте). 2 — камера заметно отстаёт, догоняет с 'ленцой'. 4 — было раньше, при резком развороте (~100°) камера ощутимо не успевала. 7 (дефолт) — успевает за резким дрифтом без видимого лага, но не дёргается. 10+ — почти мгновенно разворачивается, как приклеенная." },
