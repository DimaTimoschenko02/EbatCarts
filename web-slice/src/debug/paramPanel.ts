...
const PARAM_DESCRIPTORS: ParamDescriptor[] = [
  ...
  { key: "driftVisualOffsetDeg", group: "Дрифт v4.0 (continuous)", min: 5, max: 45, step: 1, unit: "°",
    short: "На сколько градусов кузов визуально 'кладётся' в дрифте относительно направления движения. Чисто декоративный офсет — на реальное направление езды НЕ влияет. 10° (дефолт) — лёгкий читаемый занос, при отпускании руля почти не заметно как он 'отматывается' назад. 39° — эффектный занос как в SmashKarts, но при отпускании руля кузов на глаз ощутимо 'довозвращает нос' пока офсет тает — если это раздражает, держи значение маленьким. 45° — почти боком." },
